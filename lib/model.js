 ...
 * @returns {function}
 */
function createClosure(rule, params) {
...
function Model(options) {
        ...
        if (typeof objDefinitions === "object" && objDefinitions !== null) {
            ...
            for (var key in objDefinitions) {
                ...
                if (objDefinition) {
                    ...
                    validation = self._setValidationRule(objDefinition.validation);
     ...
     *
     * @param {type} arrValidation
     * @returns {Array}
     */
    self._setValidationRule = function _setValidationRule(arrValidation) {
        ...
        for(var i = 0; i < arrValidation.length; i++) {
            ...
            if(ruleType === "function") {
                /* We're passing a function to validate against */
                ...
                if(rule !== null) {
                    ...
                    if(typeof validationutils[rule] !== "function") {
                        throw new Error(obj.rule + " is not a validation function");
                    } else {
                        rule = validationutils[rule];
                    }
            ...
            if(rule) {
                ...
                var fn = createClosure(rule, obj.param);
...
_.extend(Model.prototype, {
    ...
    validate: function() {
        ...
        for(var key in this._definition) {
            ...
            var arrValidate = this._definition[key].validation;
            ...
            for(var i = 0; i < arrValidate.length; i++) {
                ...
                try {
                    ...
                    objValidationError.addError(key, {
                        message: err.message,
                        value: err.value
                    });
